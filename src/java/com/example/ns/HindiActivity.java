...
public class HindiActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.mButtonChooseImage.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                HindiActivity.this.openFileChooser();
            }
        });
        this.mButtonUpload.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (HindiActivity.this.mUploadTask == null || !HindiActivity.this.mUploadTask.isInProgress()) {
                    HindiActivity.this.uploadFile();
                } else {
                    Toast.makeText(HindiActivity.this, "Upload in progress", 0).show();
                }
            }
        });
        this.mTextViewShowUploads.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                HindiActivity.this.openImagesActivity1();
            }
        });
    }
            ...
            this.mUploadTask = fileReference.child(stringBuilder.toString()).putFile(this.mImageUri).addOnSuccessListener(new OnSuccessListener<TaskSnapshot>() {
                public void onSuccess(TaskSnapshot taskSnapshot) {
                    new Handler().postDelayed(new Runnable() {
                        public void run() {
                            HindiActivity.this.mProgressBar.setProgress(0);
                        }
                    }, 500);
                    Toast.makeText(HindiActivity.this, "Upload successful", 1).show();
                    Task<Uri> urlTask = taskSnapshot.getStorage().getDownloadUrl();
                    while (!urlTask.isSuccessful()) {
                    }
                    HindiActivity.this.mDatabaseRef.child(HindiActivity.this.mDatabaseRef.push().getKey()).setValue(new Upload(HindiActivity.this.mEditTextFileName.getText().toString().trim(), ((Uri) urlTask.getResult()).toString()));
                }
            }).addOnFailureListener(new OnFailureListener() {
                public void onFailure(Exception e) {
                    Toast.makeText(HindiActivity.this, e.getMessage(), 0).show();
                }
                ...
                public void onProgress(TaskSnapshot taskSnapshot) {
                    ...
                    HindiActivity.this.mProgressBar.setProgress((int) (progress / totalByteCount));
                }
            });
            return;
        }
    ...
    private void openImagesActivity1() {
        startActivity(new Intent(this, ImagesActivity1.class));
    }
}

...
public class MathsActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.mButtonChooseImage.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                MathsActivity.this.openFileChooser();
            }
        });
        this.mButtonUpload.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (MathsActivity.this.mUploadTask == null || !MathsActivity.this.mUploadTask.isInProgress()) {
                    MathsActivity.this.uploadFile();
                } else {
                    Toast.makeText(MathsActivity.this, "Upload in progress", 0).show();
                }
            }
        });
        this.mTextViewShowUploads.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                MathsActivity.this.openImagesActivity3();
            }
        });
    }
            ...
            this.mUploadTask = fileReference.child(stringBuilder.toString()).putFile(this.mImageUri).addOnSuccessListener(new OnSuccessListener<TaskSnapshot>() {
                public void onSuccess(TaskSnapshot taskSnapshot) {
                    new Handler().postDelayed(new Runnable() {
                        public void run() {
                            MathsActivity.this.mProgressBar.setProgress(0);
                        }
                    }, 500);
                    Toast.makeText(MathsActivity.this, "Upload successful", 1).show();
                    Task<Uri> urlTask = taskSnapshot.getStorage().getDownloadUrl();
                    while (!urlTask.isSuccessful()) {
                    }
                    MathsActivity.this.mDatabaseRef.child(MathsActivity.this.mDatabaseRef.push().getKey()).setValue(new Upload(MathsActivity.this.mEditTextFileName.getText().toString().trim(), ((Uri) urlTask.getResult()).toString()));
                }
            }).addOnFailureListener(new OnFailureListener() {
                public void onFailure(Exception e) {
                    Toast.makeText(MathsActivity.this, e.getMessage(), 0).show();
                }
                ...
                public void onProgress(TaskSnapshot taskSnapshot) {
                    ...
                    MathsActivity.this.mProgressBar.setProgress((int) (progress / totalByteCount));
                }
            });
            return;
        }
    ...
    private void openImagesActivity3() {
        startActivity(new Intent(this, ImagesActivity3.class));
    }
}

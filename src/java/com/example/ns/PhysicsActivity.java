...
public class PhysicsActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.mButtonChooseImage.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                PhysicsActivity.this.openFileChooser();
            }
        });
        this.mButtonUpload.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (PhysicsActivity.this.mUploadTask == null || !PhysicsActivity.this.mUploadTask.isInProgress()) {
                    PhysicsActivity.this.uploadFile();
                } else {
                    Toast.makeText(PhysicsActivity.this, "Upload in progress", 0).show();
                }
            }
        });
        this.mTextViewShowUploads.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                PhysicsActivity.this.openImagesActivity5();
            }
        });
    }
            ...
            this.mUploadTask = fileReference.child(stringBuilder.toString()).putFile(this.mImageUri).addOnSuccessListener(new OnSuccessListener<TaskSnapshot>() {
                public void onSuccess(TaskSnapshot taskSnapshot) {
                    new Handler().postDelayed(new Runnable() {
                        public void run() {
                            PhysicsActivity.this.mProgressBar.setProgress(0);
                        }
                    }, 500);
                    Toast.makeText(PhysicsActivity.this, "Upload successful", 1).show();
                    Task<Uri> urlTask = taskSnapshot.getStorage().getDownloadUrl();
                    while (!urlTask.isSuccessful()) {
                    }
                    PhysicsActivity.this.mDatabaseRef.child(PhysicsActivity.this.mDatabaseRef.push().getKey()).setValue(new Upload(PhysicsActivity.this.mEditTextFileName.getText().toString().trim(), ((Uri) urlTask.getResult()).toString()));
                }
            }).addOnFailureListener(new OnFailureListener() {
                public void onFailure(Exception e) {
                    Toast.makeText(PhysicsActivity.this, e.getMessage(), 0).show();
                }
                ...
                public void onProgress(TaskSnapshot taskSnapshot) {
                    ...
                    PhysicsActivity.this.mProgressBar.setProgress((int) (progress / totalByteCount));
                }
            });
            return;
        }
    ...
    private void openImagesActivity5() {
        startActivity(new Intent(this, ImagesActivity5.class));
    }
}

    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.mButtonUpload.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (TeluguActivity.this.mUploadTask == null || !TeluguActivity.this.mUploadTask.isInProgress()) {
                    TeluguActivity.this.uploadFile();
                } else {
                    Toast.makeText(TeluguActivity.this, "Upload in progress", 0).show();
                }
            }
        });
        this.mButtonChooseImage.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                TeluguActivity.this.openFileChooser();
            }
        });
        this.mTextViewShowUploads.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                TeluguActivity.this.openImagesActivity();
            }
        });
    }
            ...
            this.mUploadTask = fileReference.child(stringBuilder.toString()).putFile(this.mImageUri).addOnSuccessListener(new OnSuccessListener<TaskSnapshot>() {
                public void onSuccess(TaskSnapshot taskSnapshot) {
                    new Handler().postDelayed(new Runnable() {
                        public void run() {
                            TeluguActivity.this.mProgressBar.setProgress(0);
                        }
                    }, 500);
                    Toast.makeText(TeluguActivity.this, "Upload successful", 1).show();
                    Task<Uri> urlTask = taskSnapshot.getStorage().getDownloadUrl();
                    while (!urlTask.isSuccessful()) {
                    }
                    TeluguActivity.this.mDatabaseRef.child(TeluguActivity.this.mDatabaseRef.push().getKey()).setValue(new Upload(TeluguActivity.this.mEditTextFileName.getText().toString().trim(), ((Uri) urlTask.getResult()).toString()));
                }
            }).addOnFailureListener(new OnFailureListener() {
                public void onFailure(Exception e) {
                    Toast.makeText(TeluguActivity.this, e.getMessage(), 0).show();
                }
                ...
                public void onProgress(TaskSnapshot taskSnapshot) {
                    ...
                    TeluguActivity.this.mProgressBar.setProgress((int) (progress / totalByteCount));
                }
            });
            return;
        }

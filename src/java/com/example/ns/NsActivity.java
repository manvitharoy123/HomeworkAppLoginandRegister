...
public class NsActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.mButtonChooseImage.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                NsActivity.this.openFileChooser();
            }
        });
        this.mButtonUpload.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (NsActivity.this.mUploadTask == null || !NsActivity.this.mUploadTask.isInProgress()) {
                    NsActivity.this.uploadFile();
                } else {
                    Toast.makeText(NsActivity.this, "Upload in progress", 0).show();
                }
            }
        });
        this.mTextViewShowUploads.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                NsActivity.this.openImagesActivity4();
            }
        });
    }
            ...
            this.mUploadTask = fileReference.child(stringBuilder.toString()).putFile(this.mImageUri).addOnSuccessListener(new OnSuccessListener<TaskSnapshot>() {
                public void onSuccess(TaskSnapshot taskSnapshot) {
                    new Handler().postDelayed(new Runnable() {
                        public void run() {
                            NsActivity.this.mProgressBar.setProgress(0);
                        }
                    }, 500);
                    Toast.makeText(NsActivity.this, "Upload successful", 1).show();
                    Task<Uri> urlTask = taskSnapshot.getStorage().getDownloadUrl();
                    while (!urlTask.isSuccessful()) {
                    }
                    NsActivity.this.mDatabaseRef.child(NsActivity.this.mDatabaseRef.push().getKey()).setValue(new Upload(NsActivity.this.mEditTextFileName.getText().toString().trim(), ((Uri) urlTask.getResult()).toString()));
                }
            }).addOnFailureListener(new OnFailureListener() {
                public void onFailure(Exception e) {
                    Toast.makeText(NsActivity.this, e.getMessage(), 0).show();
                }
                ...
                public void onProgress(TaskSnapshot taskSnapshot) {
                    ...
                    NsActivity.this.mProgressBar.setProgress((int) (progress / totalByteCount));
                }
            });
            return;
        }
    ...
    private void openImagesActivity4() {
        startActivity(new Intent(this, ImagesActivity4.class));
    }
}
